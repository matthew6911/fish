...
class Program
{
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (CheckWin())
                {
                    Console.Clear();
                    ShowField();
                    Console.WriteLine($"игрок {player} победил! Молодец!");
                    break;
                }
                else if (turns == 9)
                {
            ...
            Console.WriteLine("еще раз? (пиши yes/no)");
            string answer = Console.ReadLine().ToLower();
            if (answer != "yes") break;
        }

        Console.WriteLine("Удачи жми любую кнопку чтоб уйти...");
        Console.ReadKey();
    }
    ...
    static void BotMove()
    {
        Console.WriteLine("бот продумывает тактику...");
        int row, col;
        do
        {
